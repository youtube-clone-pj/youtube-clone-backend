-- =====================================================
-- WebSocketSimulation 성능 테스트용 최소 데이터 생성 스크립트
-- =====================================================
-- MySQL 8.0+ 필수 (Recursive CTE 사용)
-- WebSocketSimulation에 필요한 데이터만 생성:
--   - Users: 10,000명
--   - Channels: 10,000개 (각 유저당 1개)
--   - LiveStreaming: 1개 (ID=1, WebSocket 테스트용)
--   - LiveStreamingChat: 30개 (초기 메시지 로드용)
-- =====================================================
...
-- =====================================================
-- 5. LiveStreaming 테이블: 1개 생성 (ID=1, WebSocket 테스트용)
-- =====================================================

-- 첫 번째 채널에 LIVE 상태 스트리밍 생성
INSERT INTO live_streaming (id, channel_id, title, description, thumbnail_url, status, created_date, last_modified_date)
VALUES (
    1,
    1,
    'WebSocket 성능 테스트용 라이브 스트리밍',
    '대규모 동시 접속 WebSocket 채팅 테스트',
    'https://storage.youtube.com/thumbnail/1.jpg',
    'LIVE',
    NOW(),
    NOW()
);
...
SELECT '✓ LiveStreaming 생성 완료: 1개 (ID=1)' AS status;

-- =====================================================
-- 6. LiveStreamingChat 테이블: 30개 생성 (초기 메시지 로드용)
-- =====================================================

-- LiveStreaming ID=1에 대해 30개의 초기 채팅 메시지 생성
-- 다양한 사용자가 작성한 메시지로 구성
...
WITH RECURSIVE chat_seq AS (
    SELECT 1 AS n
    UNION ALL
    SELECT n + 1 FROM chat_seq WHERE n < 30
)
SELECT
    1 AS live_streaming_id,
    n AS user_id,
    CASE
        WHEN n % 5 = 0 THEN '안녕하세요! 반갑습니다'
        WHEN n % 5 = 1 THEN 'ㄱㄱㄱㄱ'
        WHEN n % 5 = 2 THEN '왔다!'
        WHEN n % 5 = 3 THEN '시작!'
        ELSE '오~'
    END AS message,
    'CHAT' AS message_type,
    DATE_SUB(NOW(), INTERVAL (30 - n) SECOND) AS created_date,
    NOW() AS last_modified_date
FROM chat_seq;

COMMIT;

SELECT '✓ LiveStreamingChat 생성 완료: 30개 (초기 메시지 로드용)' AS status;
...
SELECT '
=====================================================
✅ WebSocketSimulation 테스트 데이터 생성 완료!
=====================================================
- Users: 10,000명
- Channels: 10,000개 (각 유저당 1개)
- Live Streaming: 1개 (ID=1, LIVE 상태, WebSocket 테스트용)
- Live Streaming Chat: 30개 (초기 메시지 로드용)
=====================================================
...
WebSocket 연결 대상:
  - LiveStreaming ID: 1
  - 채팅 Topic: /topic/livestreams/1/chat/messages
  - 초기 메시지: 30개 (최근 30개 메시지 조회)
=====================================================
' AS summary;